...
void Draw_CheatsMenu_Options_Ani(void)
{
    ...
    for (ani = 0; ani < MENU_ANI_MAX; ani++)
    {
        ...
        DrawScrollBar(menu_old_sel[7], list_count(selected_centry->options[option_index].opts), APP_LINE_OFFSET, SCREEN_WIDTH - 60, game_a);
...
void Draw_CheatsMenu_View_Ani(const char* title)
{
    ...
    for (ani = 0; ani < MENU_ANI_MAX; ani++)
    {
		...
		DrawScrollBar(menu_old_sel[6], nlines, APP_LINE_OFFSET, SCREEN_WIDTH - 60, game_a);
