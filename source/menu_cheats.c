...
void Draw_CheatsMenu_Options_Ani(void)
{
    ...
    for (ani = 0; ani < MENU_ANI_MAX; ani++)
    {
        ...
        DrawScrollBar(menu_old_sel[MENU_CODE_OPTIONS], list_count(selected_centry->options[option_index].opts), APP_LINE_OFFSET, SCREEN_WIDTH - 60, game_a);
...
void Draw_CheatsMenu_Options(void)
{
	...
	DrawScrollBar(menu_sel, list_count(selected_centry->options[option_index].opts), APP_LINE_OFFSET, SCREEN_WIDTH - 60, 0xFF);
}
...
int DrawCodes(code_entry_t* code, u8 alpha, int y_inc, int xOff, int selIndex)
{
    ...
    if (code->file && (code->type == PATCH_BSD || code->type == PATCH_GAMEGENIE))
        DrawFormatString(xOff + MENU_ICON_OFF + 20, 440, "%s %s", _("Target File:"), code->file);
    ...
    for (c = startDrawX; c < max; c++)
    {
        if (c >= 0 && c < numOfLines)
        {
            ...
            //Selector
            if (c == selIndex)
                DrawTexture(&menu_textures[mark_line_png_index], xOff, yOff, 0, SCREEN_WIDTH - xOff, menu_textures[mark_line_png_index].height * 3/2, 0xFFFFFF00 | alpha);
        }
        yOff += y_inc;
    }
...
void Draw_CheatsMenu_View_Ani(const char* title)
{
    ...
    for (ani = 0; ani < MENU_ANI_MAX; ani++)
    {
		...
		DrawScrollBar(menu_old_sel[MENU_PATCH_VIEW], nlines, APP_LINE_OFFSET, SCREEN_WIDTH - 60, game_a);
...
void Draw_CheatsMenu_View(const char* title)
{
    ...
    DrawScrollBar(menu_sel, nlines, APP_LINE_OFFSET, SCREEN_WIDTH - 60, 0xFF);
}
    ...
    for (; x < max; x++)
    {
        if (x >= 0 && node)
        {
			...
			if (item->name)
			{
				...
				while ((game_name_width = WidthFromStr(nBuffer)) > 0 && (MENU_ICON_OFF + (MENU_TITLE_OFF * 1) + game_name_width) > ((SCREEN_WIDTH - 40) - (MENU_ICON_OFF * 4)))
			...
			tmp[2] = (item->type == FILE_TYPE_VMC) ? CHAR_TAG_VMC : ' ';
...
skip_draw:
        ...
        if (x == selIndex)
        {
            DrawTexture(&menu_textures[mark_line_png_index], 0, game_y, 0, SCREEN_WIDTH, menu_textures[mark_line_png_index].height * 3/2, 0xFFFFFF00 | alpha);
    ...
    DrawScrollBar(selIndex, list_count(games), y_inc, SCREEN_WIDTH - 60, alpha);
}
...
skip_code:
    ...
    DrawScrollBar(selIndex, list_count(game->codes), y_inc, SCREEN_WIDTH - 60, alpha);
}
