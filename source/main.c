...
int pad_input_update(void *data)
{
	...
	while (!close_app)
	{
		sceCtrlPeekBufferPositive(0, &padA[0], 1);
//		scePadReadState(padhandle, &padA[0]);
		...
		if(g_padSync)
		{
			...
			if (padA[0].ly < ANALOG_MIN)
				input->down |= SCE_CTRL_UP;

			if (padA[0].ly > ANALOG_MAX)
				input->down |= SCE_CTRL_DOWN;

			if (padA[0].lx < ANALOG_MIN)
				input->down |= SCE_CTRL_LEFT;

			if (padA[0].lx > ANALOG_MAX)
...
void doSaveMenu(save_list_t * save_list)
{
    if (pad_sync())
    {
    	if(pad_data.active & SCE_CTRL_UP)
    		move_selection_back(list_count(save_list->list), 1);
    
    	else if(pad_data.active & SCE_CTRL_DOWN)
    		move_selection_fwd(list_count(save_list->list), 1);
    
    	else if (pad_data.active & SCE_CTRL_LEFT)
    	...
    	else if (pad_check_button(SCE_CTRL_CROSS))
    	{
    	...
    	else if (pad_check_button(SCE_CTRL_TRIANGLE) && save_list->UpdatePath)
    	{
			selected_entry = list_get_item(save_list->list, menu_sel);
		...
		else if (pad_check_button(SCE_CTRL_SELECT) && 
		...
		else if (pad_check_button(SCE_CTRL_SQUARE))
		{
...
void doMainMenu()
{
	...
	if (pad_sync())
	{
		if(pad_data.active & SCE_CTRL_LEFT)
		...
		else if(pad_check_button(SCE_CTRL_CIRCLE) && show_dialog(1, "Exit to XMB?"))
			close_app = 1;
	}
...
void doPatchMenu()
{
	...
	if (pad_sync())
	{
		...
		else if (pad_check_button(SCE_CTRL_TRIANGLE))
		{
			selected_centry = list_get_item(selected_entry->codes, menu_sel);
