...
int load_modules(void) {
	...
	if (ret) {
		LOG("sceSysmoduleLoadModuleInternal() failed: 0x%08X\n", ret);
	}
...
static int s_bgft_initialized = 0;
...
int bgft_init(void) {
	...
	if (ret) {
		LOG("sceBgftServiceIntInit failed: 0x%08X\n", ret);
		goto err_bgft_heap_free;
	}

	s_bgft_initialized = 1;
...
void bgft_fini(void) {
	...
	if (ret) {
		LOG("sceBgftServiceIntTerm failed: 0x%08X\n", ret);
	}
...
int bgft_download_pkg(const char* pkg_url)
{
	int ret, user_id;

	sceUserServiceGetForegroundUser(&user_id);
	
	LOG("New install task: %s\n", pkg_url);
	...
	memset(&download_params, 0, sizeof(download_params));
	{
		...
		download_params.params.contentUrl = "/data/IV0000-APOL00004_00-APOLLO0000000PS4.pkg";
		download_params.params.contentName = "IV0000-APOL00004_00-APOLLO0000000PS4.pkg";
	...
	OrbisBgftDownloadRegisterErrorInfo errz;
	memset(&errz, 0, sizeof(OrbisBgftDownloadRegisterErrorInfo));

	int task_id = -1;
	ret = sceBgftServiceIntDownloadRegisterTaskByStorageEx(&download_params, &task_id);
	LOG("[%s]", errz.buf);
	if (ret) {
		LOG("Cannot install %s: BGFT Error 0x%08X\n", pkg_url, ret);
		goto err;
	}
	LOG("Task ID: 0x%08X\n", task_id);
	...
	if (ret) {
		LOG("sceBgftServiceDownloadStartTask failed: 0x%08X\n", ret);
		goto err;
	}
...
void bg_test()
{
	load_modules();
	bgft_init();
	bgft_download_pkg("http://www.bucanero.com.ar/IV0000-APOL00004_00-APOLLO0000000PS4.pkg");
	bgft_fini();
}
*/
